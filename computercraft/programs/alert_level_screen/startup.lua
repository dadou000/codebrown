-- Dynamic MCCR startup for alert_level_screen. Bundled dynamic program.
-- Concrete instance is read from /mccr_device.dat.
local MCCR_PROGRAM = "alert_level_screen"
local MCCR_VERSION = "1.0.1"
local MCCR_ROLE = "display"
local MCCR_DEFAULT_NAME = "alert_level_screen"
...
local function drawMonitor(t, lib, snap, name)
  local w = lib.ui.size(t)
  ...
  elseif mode == "draconic" then
    drawDeviceList(t, lib, snap, "Draconic", function(n, l) return n:find("draconic") or l:lower():find("draconic") end)
    return
...
local function run(name, lib)
  ...
  while true do
    ...
    elseif spec.display == "monitor" then s.sleepTick = 0; drawMonitor(screen, lib, s.snapshot, name)
