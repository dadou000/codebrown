-- Dynamic MCCR startup for PMC. Bundled dynamic program.
-- Concrete instance is read from /mccr_device.dat.
local MCCR_PROGRAM = "PMC"
local MCCR_VERSION = "1.0.1"
local MCCR_ROLE = "pmc"
local MCCR_DEFAULT_NAME = "PMC1"
...
local function load_role_pmc()
local pages = { "Batteries", "Voltages", "Amps/Watts", "Breakers", "Actions" }
...
local function command(lib, name, payload)
...
local function run(name, lib)
  ...
  local s = lib.state.read(statePath, { page = 1, snapshot = {}, selectedBattery = nil, eval = {} })
  ...
  local function drawTabs()
    ...
    for i, title in ipairs(pages) do
      local label = w < 45 and tostring(i) or title
      local bw = w < 45 and math.max(2, math.floor((w - 4) / #pages)) or math.max(8, #title + 2)
      table.insert(buttons, lib.ui.button(screen, "page:" .. i, x, 2, bw, label, colors.white, i == s.page and colors.blue or colors.gray))
      x = x + bw + 1
    end
  end
  ...
  local function draw()
    ...
    local eval = lib.power.evaluate(spec, lib.power.inputFor(spec, snap), s.eval)
    ...
    lib.ui.center(screen, 1, name .. " POWER", colors.lightBlue)
    drawTabs()
    lib.ui.writeAt(screen, 1, 3, "Self " .. eval.status .. string.format(" %0.1fV", eval.voltage), lib.ui.statusColor(eval.severity))

    local y = 4
    if s.page == 1 then
      if s.selectedBattery then
        local pct = (snap.batteries or {})[s.selectedBattery] or 0
        table.insert(buttons, lib.ui.button(screen, "back", 1, y, math.min(8, w), "BACK", colors.white, colors.gray))
        y = y + 1
        lib.ui.writeAt(screen, 1, y, string.upper(s.selectedBattery) .. " detail", colors.yellow)
        lib.ui.writeAt(screen, 1, y + 1, string.format("State of charge: %0.1f%%", pct), colors.white)
        lib.ui.bar(screen, 1, y + 2, w, pct, 100, pct < 20 and colors.red or colors.green)
        lib.ui.writeAt(screen, 1, y + 4, "PMC 500Wh; 400V 500kWh", colors.gray)
      else
        for _, key in ipairs({ "battery400", "pmc1", "pmc2", "pmc3" }) do
          local pct = (snap.batteries or {})[key] or 0
          local label = (key == "battery400" and "400V" or key)
          lib.ui.writeAt(screen, 1, y, label .. " " .. string.format("%0.0f%%", pct), pct < 20 and colors.red or colors.green)
          if w >= 20 then
            lib.ui.bar(screen, 12, y, w - 11, pct, 100, pct < 20 and colors.red or colors.green)
          end
          table.insert(buttons, { id = "battery:" .. key, x = 1, y = y, w = w, h = 1 })
          y = y + 1
          if y > h then break end
        end
      end
    elseif s.page == 2 then
      for _, key in ipairs({ "ac230", "ac400", "dc12", "pmc1", "pmc2", "pmc3" }) do
        local bus = (snap.buses or {})[key] or {}
        lib.ui.writeAt(screen, 1, y, string.format("%s %0.2fV", key, bus.voltage or 0), (bus.voltage or 0) <= 0 and colors.red or colors.green)
        y = y + 1
        if y > h then break end
      end
    elseif s.page == 3 then
      ...
      lib.ui.writeAt(screen, 1, y, string.format("Gen %0.2fMW", gen / 1000000), colors.green)
      lib.ui.writeAt(screen, 1, y + 1, string.format("Load %0.2fMW", load / 1000000), colors.yellow)
      lib.ui.writeAt(screen, 1, y + 2, string.format("AC %0.1fA", load / math.max(1, (((snap.buses or {}).ac230 or {}).voltage or 230))), colors.white)
    elseif s.page == 4 then
      local keys = sortedKeys(snap.breakers or {})
      for _, key in ipairs(keys) do
        if y <= h then
          local on = snap.breakers[key]
          local label = key .. ": " .. (on and "CLOSED" or "OPEN")
          lib.ui.writeAt(screen, 1, y, label, on and colors.green or colors.red)
          table.insert(buttons, { id = "breaker:" .. key, x = 1, y = y, w = w, h = 1 })
          y = y + 1
        end
      end
    elseif s.page == 5 then
      ...
      lib.ui.writeAt(screen, 1, y, "Recommended:", colors.yellow)
      y = y + 1
      if level >= 5 then
        lib.ui.writeAt(screen, 1, y, "Evacuate; emergency power only.", colors.purple)
      elseif level >= 4 then
        lib.ui.writeAt(screen, 1, y, "SCRAM and shed load.", colors.red)
      elseif level >= 3 then
        lib.ui.writeAt(screen, 1, y, "Isolate branch; prep shed.", colors.orange)
      elseif level >= 2 then
        lib.ui.writeAt(screen, 1, y, "Watch trend; verify buses.", colors.yellow)
      else
        lib.ui.writeAt(screen, 1, y, "No action required.", colors.green)
      end
      local by = math.min(h, y + 2)
      table.insert(buttons, lib.ui.button(screen, "scram", 1, by, math.min(7, w), "SCRAM", colors.white, colors.red))
      table.insert(buttons, lib.ui.button(screen, "shed", math.min(w, 9), by, math.max(0, w - 8), "SHED", colors.white, colors.orange))
    end
  end
  ...
  local function click(id)
    ...
    if id:find("^page:") then
      s.page = tonumber(id:sub(6)) or 1
      s.selectedBattery = nil
    elseif id == "back" then
      s.selectedBattery = nil
    elseif id:find("^battery:") then
      s.selectedBattery = id:sub(9)
    elseif id:find("^breaker:") then
      command(lib, name, { command = "toggle_breaker", breaker = id:sub(9) })
    elseif id == "scram" then
      command(lib, name, { command = "scram" })
    elseif id == "shed" then
      command(lib, name, { command = "shed" })
    end
  end
  ...
  while true do
    ...
    while true do
      ...
      if ev[1] == "rednet_message" then
        local pkt = ev[3]
        if type(pkt) == "table" and pkt.system == "mccr" and pkt.kind == "command" and pkt.payload and mccrHandleBootloaderInstall(lib, name, pkt.payload) then
          -- handled by firmware installer
        elseif type(pkt) == "table" and pkt.system == "mccr" and pkt.kind == "snapshot" then
